...
def calculate_dividend_weights(dividends):
    ...
    """
    d_cumsum = dividends.cumsum()
    dividend_weights = d_cumsum.divide(d_cumsum.sum(axis=1),axis=0)
...
def calculate_cumulative_returns(returns):
    ...
    """
    #TODO: Implement function
    cum_return = (returns.sum(axis=1)+1).cumprod()
...
def get_optimal_weights(covariance_returns, index_weights, scale=2.0):
    ...
    covariance = cvx.quad_form(x, covariance_returns)
    
    index_dist = cvx.norm(x- index_weights, p=2)
    
    objective = cvx.Minimize(covariance + scale * index_dist)
...
def get_portfolio_turnover(all_rebalance_weights, shift_size, rebalance_count, n_trading_days_in_year=252):
    ...
    weight = np.diff(np.flip(all_rebalance_weights,axis=0),axis=0)
    total_turnover = np.abs(weight).sum()
