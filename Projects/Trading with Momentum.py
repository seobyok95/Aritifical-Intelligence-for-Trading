...
def get_top_n(prev_returns, top_n):
    ...
    """
    # TODO: Implement Function
    best_stocks = pd.DataFrame(0,index=prev_returns.index, columns=prev_returns.columns)
    for date, returns in prev_returns.iterrows():
        best_stocks.loc[date,returns.nlargest(top_n).index] = 1
    return best_stocks.astype('int')
...
def portfolio_returns(df_long, df_short, lookahead_returns, n_stocks):
    ...
    """
    # TODO: Implement Function
    long = lookahead_returns.mask(~df_long.astype(bool)).fillna(0)
    short = lookahead_returns.mask(~df_short.astype(bool)).fillna(0)
    
    return (long-short)/n_stocks
...
# With an observed p-value = 0.073339 > 0.05, we fail to reject the null hypothesis, and accept the alternative hypothesis at alpha = 0.05. This suggests that the results are not statistically significant and the strategy not profitable. 
